...
def log_metrics(dataset_name, records_received, records_processed):
    records_dropped = records_received - records_processed
...
if __name__ == "__main__":
    ...
    #Caputer Metric 1: row count
    book_records_received = len(df_system_book)
    customer_records_received = len(df_system_customers)
    ...
    ##check Duplicates
    df_system_book, book_duplicate_rows_removed = duplicateCheck(df_system_book)
    ...
    book_null_rows_removed = (df_system_book.isnull().any(axis=1).sum())

    #Data METRICS
    book_records_processed = len(df_system_book)
    book_records_dropped = book_records_received - book_records_processed
    book_null_rows_removed = len(error_log)
    ...
    etl_metrics.append({
    "Run Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "Dataset": "Library System Book",
    "Records Received": book_records_received,
    "Records Processed": book_records_processed,
    "Records Dropped": book_records_dropped,
    "Null Rows Removed": book_null_rows_removed,
    "Duplicate Rows Removed": book_duplicate_rows_removed})


    #DATA SET 2- CLEANING
    ##Remove all nulls
    df_system_customers = naCheck(df_system_customers)
    customer_null_rows_removed = (customer_records_received - len(df_system_customers))
   ## print("After naCheck:", type(df_system_customers))

    ##check Duplicates
   # df_system_customers = duplicateCheck(df_system_customers)
    #df_system_customers, customer_duplicate_rows_removed = duplicateCheck(df_system_customers)
    df_system_customers, customer_duplicate_rows_removed = duplicateCheck(df_system_customers)
  
  ##  print("After duplicateCheck:", type(df_system_customers))

    #Data METRICS
    customer_records_processed = len(df_system_customers)
    customer_records_dropped = (customer_records_received - customer_records_processed)

    #DATA SET 2- VALIDATION & ERROR LOGGING
    print(df_system_customers)
    print(type(df_system_customers))
   ...
   ##Log missing IDS for review
    ...
    etl_metrics.append({
    "Run Date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "Dataset": "Library System Customers",
    "Records Received": customer_records_received,
    "Records Processed": customer_records_processed,
    "Records Dropped": customer_records_dropped,
    "Null Rows Removed": customer_null_rows_removed,
    "Duplicate Rows Removed": customer_duplicate_rows_removed})
   ...
   #DATA SET 2- CLEAN OUTPUT
    ...
    metrics_df = pd.DataFrame(etl_metrics)

    #Calculate run time
    end_time = time.time()
    run_time_seconds = round(end_time - start_time, 2)
    print(f"Pipeline Run Time: {run_time_seconds} seconds")

    #Add run time metrics to table and output to csv
    metrics_df["Run Time (Seconds)"] = run_time_seconds
    metrics_df.to_csv("03_ETL_Metrics.csv", index=False)
    
    print("ETL Metrics File saved")
